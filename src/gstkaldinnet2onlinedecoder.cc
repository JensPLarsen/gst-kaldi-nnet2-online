    ...
    Gstkaldinnet2onlinedecoder *filter, CompactLattice &clat)
{
  if (filter->inverse_scale)
  {
    ...
    if (filter->nnet_mode == NNET2)
    {
      if (filter->use_threaded_decoder)
      {
        inv_acoustic_scale = 1.0 / filter->nnet2_decoding_threaded_config->acoustic_scale;
      }
      else
      {
        inv_acoustic_scale = 1.0 / filter->nnet2_decoding_config->decodable_opts.acoustic_scale;
      }
    }
    else
    {
                                                                  ...
                                                                  BaseFloat traceback_period_secs)
{
  ...
  while (true)
  {
    ...
    if (filter->do_endpointing && (decoder.NumFramesDecoded() > 0) && decoder.EndpointDetected(*(filter->endpoint_config)))
    {
      GST_DEBUG_OBJECT(filter, "Endpoint detected!");
      break;
    }
                                         ...
                                         const GValue *value)
{
  if (G_VALUE_HOLDS_POINTER(value))
  {
    ...
    {
      GST_WARNING_OBJECT(filter, "Error loading the FST decoding graph pointer");
    }
  }
  else
  {
    GST_WARNING_OBJECT(filter, "FST property must be a pointer to a Fst<fst::StdArc>. Ignoring it.");
  }
}
